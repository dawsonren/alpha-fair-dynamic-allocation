...
class TestAllocationSolver(unittest.TestCase):
    ...
    def test_monte_carlo(self):
        ...
        def test_monte_carlo_instance(prob: AllocationSolver):
            n = 1 # hoeffding_bound(0.01, 0.01)
            Z, w = prob.solve()
            metrics = prob.monte_carlo_performance_metrics_batched(n)
            print(Z, (metrics["social_welfare"][0] + metrics["social_welfare"][1]) / 2)
            print(w, (metrics["waste"][0] + metrics["waste"][1]) / 2)

            self.assertAlmostEqual((metrics["social_welfare"][0] + metrics["social_welfare"][1]) / 2, Z, delta=0.01)
            self.assertAlmostEqual((metrics["waste"][0] + metrics["waste"][1]) / 2, w, delta=0.01)
        ...
        random_property_test(
            lambda: generate_random_problem(2, generate_general_distribution(2), "hope_online"),
            test_monte_carlo_instance,
            instances=1
        )
